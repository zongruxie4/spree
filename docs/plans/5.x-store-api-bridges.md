...
## NOT changed in 5.x (deferred to 6.0 model rename)

### Event names

Event names (`shipment.shipped`, `shipment.canceled`) are **not changed** in 5.x. Webhook subscribers and event listeners depend on these strings. They change in 6.0 to `fulfillment.fulfilled`, `fulfillment.canceled` with old names emitted alongside for one release.
