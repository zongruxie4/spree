...
Unlike search (global) and tax (per-store), delivery rate providers are **per delivery method**. One store can have:
...
Each EasyPost-backed DeliveryMethod stores its carrier + service in metadata:
...
module SpreeEasyPost
  class DeliveryRateProvider < Spree::DeliveryRateProvider::Base
    ...
    def find_or_create_easypost_shipment(package)
      ...
      RequestStore.store[cache_key] ||= EasyPost::Shipment.create(
...
Tax doesn't use zones at all — TaxRate links directly to country/state (see `6.0-tax-provider.md`). The generic `Zone` model is removed in 6.0, replaced by `DeliveryZone` for delivery and direct country/state FKs on TaxRate for tax.
