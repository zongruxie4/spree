...
**Status:** In Progress (RefreshToken shipping in 5.5, rest in 6.0)
**Target:** Spree 5.5 (RefreshToken) + Spree 6.0 (Devise removal, User→Customer rename)
...
- **Add refresh token support (5.5).** Short-lived access JWT (15 min) + long-lived refresh token (stored in DB, rotatable, revocable). Better UX for React admin and Next.js storefront — silent refresh instead of forced re-login. Ships in 5.5 — can't wait for 6.0 because every storefront has broken session management without it.
